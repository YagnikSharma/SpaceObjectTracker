/**
 * YOLOv8 Service for Space Station Object Detection
...
/**
 * Main YOLO service for space object detection
 */
export class SpaceObjectDetector {
  ...
  constructor() {
    ...
    if (fs.existsSync(this.modelPath)) {
      log('YOLOv8 model found at: ' + this.modelPath, 'yolo');
  ...
  public loadModel(modelBuffer: Buffer): boolean {
    try {
      ...
      log(`YOLOv8 model saved to ${this.modelPath}`, 'yolo');
      ...
      log(`Error loading YOLOv8 model: ${error.message}`, 'yolo');
      return false;
    }
  }
  ...
  public detectObjects(imagePath: string) {
    ...
    if (!this.isModelLoaded) {
      if (fs.existsSync(this.modelPath)) {
        ...
        log('YOLOv8 model not loaded, using fallback detection', 'yolo');
        return this.performFallbackDetection(imagePath);
      }
    }
    
    try {
      // Process the image with our YOLOv8 model
      log(`Processing image with YOLOv8: ${imagePath}`, 'yolo');
      
      // Get detections from our precise model
      const detections = this.analyzeImage(imagePath);
      log(`Custom YOLO model detected ${detections.length} objects`, 'yolo');
  ...
  public importModel(modelBuffer: Buffer, modelName: string = 'yolov8s.pt'): boolean {
    try {
      ...
      log(`Error importing YOLOv8 model: ${error.message}`, 'yolo');
      return false;
    }
  }
